...
class tabuleiro(object):
    ...
    def __init__(auto, tela, tabuleiro, numero, tamanho):
        auto.tela = tela;
    ...
    def mostrar_tabuleiros(auto):
        auto.tela.printTo("Tabuleiro: %d" % auto.numero)
        auto.tela.printTo(" ")
        for row in auto.tabuleiro:
            auto.tela.printTo(' '.join(row))
        auto.tela.printTo(" ")
...
class batalha(object):
    def __init__(auto):
        ...
        auto.tabuleiro_um = tabuleiro(auto.tela, auto.tamanho_tabuleiro_um, 1, 4)
        auto.tabuleiro_dois = tabuleiro(auto.tela, auto.tamanho_tabuleiro_dois, 2, 4)
    ...
    def entrada_usuario(auto, linha_navio, coluna_navio, jogador, tabuleiro, tamanho_tabuleiro):
        ...
        while True:
            try:
                ...
                auto.tela.printTo("Por favor, insira apenas numeros.")
                continue
            else:
                break
        ...
        if partida:
            ...
            jogador.vitorias += 1
            auto.tela.printTo("Parabens! Voce afundou meu navio")
            auto.melhor_de(auto.mudar_estado_ganhador, jogador)
        elif ausente_tabuleiro:
            auto.tela.printTo("Oxi, voce acertou fora do mar.")
        elif tabuleiro[escolha_linha][escolha_coluna] == "X":
            auto.tela.printTo("Voce ja usou essa combinacao.")
        else:
            auto.tela.printTo("Nenhum navio foi atingido")
    ...
    def melhor_de(auto, estado_ganhador, jogador):
        if estado_ganhador == 1 and jogador.vitorias < 2:
            auto.tela.printTo("%s venceu o jogo!" % jogador.nome)
        elif auto.total_rodadas == 6:
            if estado_ganhador != 0:
                auto.tela.printTo("Este jogo empatou!")
        elif jogador.vitorias >= 2:
            auto.tela.printTo("%s ganhou a melhor de 3" % jogador.nome)
        elif jogador.derrotas >= 2:
            auto.tela.printTo("%s perdeu a melhor de 3" % jogador.nome)
